...
const SUPABASE_URL = "https://jzowyfnudqinpltcjvuy.supabase.co";
const SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp6b3d5Zm51ZHFpbnBsdGNqdnV5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njc1OTQzNDAsImV4cCI6MjA4MzE3MDM0MH0.VViZDVSVNFam7EStdLB6tNifUyeiJNuo0_9pSzjH4gI";
        ...
        list.innerHTML += `
                    <div class="card" style="background:white; border-left:4px solid #f59e0b;">
                        ...
                        <div style="background:#fff7ed; padding:10px; border-radius:8px; margin:10px 0;">
                            👤 <b>${cName}</b> (${cPlate})<br>⏱️ Gəlmə vaxtı: <b style="color:#c2410c;">${
          o.courier_eta || "?"
        }</b>
                        </div>
                        <div class="row">
                            <button onclick="app.approveCourier(${
                              o.id
                            })" class="btn btn-success" style="font-size:0.9rem;">✅ Təsdiqlə</button>
                            <button onclick="app.rejectCourier(${
                              o.id
  ...
  openSettings: () => {
    ...
    dc.innerHTML =
      app.currentRole === "seller"
        ? `<label style="font-size:0.8rem;color:#666;">Mağaza Adı:</label><input type="text" id="set-shop-name" value="${
            app.currentUser.shop_name || ""
          }"><label style="font-size:0.8rem;color:#666;">Mağaza Ünvanı:</label><input type="text" id="set-shop-address" value="${
            app.currentUser.shop_address || ""
          }">`
